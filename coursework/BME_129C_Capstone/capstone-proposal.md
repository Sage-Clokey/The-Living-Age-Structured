...
Sage Clokey, B.S. Biomedical Engineering, Spring 2026
...
Biological sequence design -- the engineering of proteins and DNA constructs for specific functions -- currently requires deep expertise across multiple domains: genomics, protein engineering, bioinformatics, and synthetic biology. The tools that exist (BLAST for homology search, UCSC Genome Browser for genome navigation, NCBI Entrez for sequence retrieval, codon optimization calculators, and protein structure predictors) are powerful individually but fragmented. Each addresses one step of the design process, speaks its own input language, and returns output that requires specialist interpretation before the next step can proceed.

This fragmentation creates a fundamental accessibility problem. A researcher who wants to design a protein with a specific biological function -- say, an enzyme that cross-links fungal cell wall polymers under mechanical stress -- must manually: (1) identify relevant gene families across multiple organisms, (2) retrieve and align reference sequences, (3) assess codon compatibility with the target expression host, (4) check for regulatory and pathway-level conflicts, and (5) interpret the results. Each of these steps requires different software, different expertise, and significant time.

There is currently no unified system that allows a researcher to describe a desired biological function in plain English and receive a designed, compatibility-scored protein sequence as output. This project proposes to build one.
...
This project extends the existing Adaptive Genome Design System with two new layers:
...
This project is the first phase of a larger research program -- the Adaptive Genome Design System -- aimed at making biological sequence design accessible to non-specialists. The long-term goal is a platform where architects, materials scientists, and community builders can design with living systems using natural language, without requiring expertise in genomics or protein engineering.

Within the academic context, this work contributes a novel integration architecture: using a large language model as a semantic bridge between human intent and protein language model sequence space, with a grounding layer (the compatibility engine) that validates outputs against real biological constraints. This architecture has not been published in the existing literature and represents a potential contribution to the fields of computational biology and AI-assisted biodesign.

The system is designed for open-source release. The compatibility engine, codon optimization tables, and bridge architecture are intended to be freely available to the synthetic biology community, including iGEM teams and academic researchers.
