...
class CellEconomyReport:
    ...
    # Comparative advantage: cell_type -> top enriched genes with fold change
    comparative_advantage: dict[str, list[tuple[str, float]]] = field(default_factory=dict)
    # Price signals: subjective value of ligands across cell types
    price_signals: list[dict] = field(default_factory=list)
    # Directed communication edges (voluntary exchange count)
    directed_comm_edges: int = 0
    # Star (centralized) network mean robustness
    star_robustness_mean: float = 0.0
    # Distributed network mean robustness
    distributed_robustness_mean: float = 0.0
...
def analyze_cell_economy(adata: "anndata.AnnData") -> CellEconomyReport:
    ...
    # --- 4. Directed communication network ---
    print("[single-cell] Building directed communication network...")
    directed_graph = _build_directed_communication_network(adata, unique_types)
    report.directed_comm_edges = directed_graph.number_of_edges()

    # --- 5. Comparative advantage via differential expression ---
    print("[single-cell] Computing comparative advantage (fold change)...")
    report.comparative_advantage = _compute_comparative_advantage(adata, unique_types)

    # --- 6. Star (centralized) network comparison ---
    print("[single-cell] Building star network and comparing robustness...")
    hub_type = unique_types[0] if unique_types else ""
    total_directed_weight = sum(
        d.get("weight", 0) for _, _, d in directed_graph.edges(data=True)
    )
    star_graph = _build_star_communication_network(
        unique_types, hub_type, total_weight=total_directed_weight
    )
    robustness_comparison = _compare_robustness(directed_graph, star_graph, unique_types)
    report.distributed_robustness_mean = robustness_comparison["distributed_mean"]
    report.star_robustness_mean = robustness_comparison["star_mean"]

    # --- 7. Price signal analysis ---
    print("[single-cell] Analyzing price signals (subjective value)...")
    report.price_signals = _analyze_price_signals(adata, unique_types)
...
def _build_directed_communication_network(
    adata: "anndata.AnnData",
    cell_types: list[str],
) -> nx.DiGraph:
    """
    Build a directed cell-cell communication network from ligand-receptor expression.

    A directed edge A->B means "cell type A produces a ligand that cell type B
    has the receptor for — A offers a signal that B values and acts on."
    This is voluntary exchange: both sides must participate (ligand expressed
    AND receptor expressed).

    Unlike the undirected version, A->B and B->A are separate edges.
    Cell type A may produce TNF (which B receives via TNFRSF1A), and separately
    B may produce TGFB1 (which A receives via TGFBR1). Two distinct voluntary
    exchanges.

    Edge attributes:
        weight: number of active L-R channels in that direction
        channels: list of pathway names
        pairs: list of (ligand, receptor) tuples
    """
    _import_scanpy()
    G = nx.DiGraph()
    ...
    for ct_a in cell_types:
        ...
        G.add_node(ct_a)
        for ct_b in cell_types:
            if ct_b not in ct_expression or ct_a == ct_b:
                continue
            G.add_node(ct_b)
            ...
            for ligand, receptor, pathway in available_pairs:
                ...
                if lig_expr > threshold and rec_expr > threshold:
                    n_channels += 1
                    active_channels.append(pathway)
                    active_pairs.append((ligand, receptor))
...
def _compute_comparative_advantage(
    adata: "anndata.AnnData",
    cell_types: list[str],
    n_top: int = 10,
) -> dict[str, list[tuple[str, float]]]:
    """
    Compute Mengerian comparative advantage via differential expression.

    For each cell type, find the genes it expresses most strongly relative
    to all other cell types combined. This is not about exports — it is about
    specialization: what each cell type uniquely contributes that others
    don't produce as well.

    Returns:
        dict mapping cell_type -> list of (gene_name, fold_change) for the
        top n_top most enriched genes.
    """
    _import_scanpy()
    ...
    for ct in cell_types:
        ...
        subset = adata[mask]
        if hasattr(subset.X, "toarray"):
            ct_means[ct] = np.array(subset.X.toarray().mean(axis=0)).flatten()
        else:
            ct_means[ct] = np.array(subset.X.mean(axis=0)).flatten()
    ...
    for ct in cell_types:
        ...
        # Mean expression across ALL other cell types combined
        other_means = [ct_means[other] for other in cell_types if other != ct and other in ct_means]
        if not other_means:
            continue
        ...
        pseudocount = 0.01
        fold_change = (ct_mean + pseudocount) / (other_mean + pseudocount)

        # Get top n_top genes by fold change
        top_indices = np.argsort(fold_change)[::-1][:n_top]
        result[ct] = [(gene_names[i], float(fold_change[i])) for i in top_indices]

    return result


def _build_star_communication_network(
    cell_types: list[str],
    hub_type: str,
    total_weight: int = 0,
) -> nx.DiGraph:
    """
    Build a star (centralized) communication network.

    The Rothbardian central planner: one bureau through which all
    coordination must pass. Every cell type connects only to the hub.
    No direct peer-to-peer communication.

    Args:
        cell_types: list of cell type names
        hub_type: the cell type that serves as the central hub
        total_weight: total edge weight to distribute proportionally
                      across all star edges. If 0, each edge gets weight 1.

    Returns:
        nx.DiGraph with star topology centered on hub_type
    """
    G = nx.DiGraph()

    spokes = [ct for ct in cell_types if ct != hub_type]
    if not spokes:
        G.add_node(hub_type)
        return G

    # Each spoke has 2 edges (spoke->hub and hub->spoke)
    n_edges = len(spokes) * 2

    if total_weight > 0:
        weight_per_edge = total_weight / n_edges
    else:
        weight_per_edge = 1
    ...
    for spoke in spokes:
        G.add_node(spoke)
        G.add_edge(spoke, hub_type, weight=weight_per_edge, channels=["centralized"], pairs=[])
        G.add_edge(hub_type, spoke, weight=weight_per_edge, channels=["centralized"], pairs=[])

    return G


def _compare_robustness(
    distributed_graph: nx.DiGraph,
    star_graph: nx.DiGraph,
    cell_types: list[str],
) -> dict:
    """
    Compare robustness of distributed vs. star (centralized) networks.

    For each cell type, remove it and measure the fraction of edges that
    survive. Decentralized networks degrade gracefully; star networks
    collapse when the hub is removed.

    Returns:
        dict with:
            distributed_robustness: {cell_type: fraction_surviving}
            star_robustness: {cell_type: fraction_surviving}
            distributed_mean: mean robustness across all removals
            star_mean: mean robustness across all removals
    """
    def _removal_robustness(graph, types):
        ...
        for ct in types:
            ...
            H = graph.copy()
            H.remove_node(ct)
            scores[ct] = H.number_of_edges() / total
        return scores

    dist_rob = _removal_robustness(distributed_graph, cell_types)
    star_rob = _removal_robustness(star_graph, cell_types)

    dist_mean = float(np.mean(list(dist_rob.values()))) if dist_rob else 0.0
    star_mean = float(np.mean(list(star_rob.values()))) if star_rob else 0.0

    return {
        "distributed_robustness": dist_rob,
        "star_robustness": star_rob,
        "distributed_mean": dist_mean,
        "star_mean": star_mean,
    }


def _analyze_price_signals(
    adata: "anndata.AnnData",
    cell_types: list[str],
    ct_expression: Optional[dict] = None,
) -> list[dict]:
    """
    Analyze price signals: Menger's subjective value in biological terms.

    The same TNF molecule means "activate" to one cell type and "die" to
    another, depending on which receptor it expresses. This is subjective
    value — the same good, different valuations by different actors.

    For each ligand in LIGAND_RECEPTOR_PAIRS:
        - Which cell types produce it (above threshold)?
        - Which cell types express each of its receptors (above threshold)?
        - What pathway/function does it serve?

    Returns:
        list of dicts: [{ligand, producers, receivers: {receptor: [cell_types]}, pathway}]
    """
    _import_scanpy()
    ...
    # Compute mean expression per cell type if not provided
    if ct_expression is None:
        ct_expression = {}
        for ct in cell_types:
            mask = adata.obs["cell_type"].astype(str) == ct
            if mask.sum() == 0:
    ...
    # Group L-R pairs by ligand
    ligand_groups: dict[str, list[tuple[str, str]]] = {}
    for ligand, receptor, pathway in LIGAND_RECEPTOR_PAIRS:
        if ligand in gene_names and receptor in gene_names:
            if ligand not in ligand_groups:
                ligand_groups[ligand] = []
            ligand_groups[ligand].append((receptor, pathway))

    signals = []
    for ligand, receptor_list in ligand_groups.items():
        # Find producers
        producers = [
            ct for ct in cell_types
            if ct in ct_expression and ct_expression[ct].get(ligand, 0) > threshold
        ]
        if not producers:
            continue

        # Find receivers per receptor
        receivers: dict[str, list[str]] = {}
        pathways = set()
        for receptor, pathway in receptor_list:
            rec_types = [
                ct for ct in cell_types
                if ct in ct_expression and ct_expression[ct].get(receptor, 0) > threshold
            ]
            if rec_types:
                receivers[receptor] = rec_types
...
def plot_cell_economy(
    ...
    if report.specialization_scores:
        ...
        ax2.barh(y_pos, scores, color=colors_bar, edgecolor=TEXT_DIM, linewidth=0.5, alpha=0.3)
        ax2.scatter(scores, y_pos, c=colors_bar, s=70, zorder=3, edgecolors="white", linewidth=0.8)
