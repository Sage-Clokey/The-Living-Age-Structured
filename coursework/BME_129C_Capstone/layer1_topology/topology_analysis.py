...
class TopologyReport:
    ...
    robustness_random_trials: list[list[float]] = field(default_factory=list)
...
def analyze_topology(G: nx.Graph, name: str = "network") -> TopologyReport:
    ...
    # --- Robustness analysis ---
    report.robustness_random, report.robustness_random_half, report.robustness_random_trials = _robustness_curve(
        U, mode="random", trials=10,
    )
    report.robustness_targeted, report.robustness_targeted_half, _ = _robustness_curve(
        U, mode="targeted",
    )
...
def _robustness_curve(
    ...
    Returns:
        (gc_fractions, half_life, gc_curves)
        ...
        gc_curves: list of individual trial curves (one per trial)
    """
    n = G.number_of_nodes()
    if n == 0:
        return [], 0.0, []
    ...
    return avg_curve, half_life, gc_curves
...
def plot_topology_report(
    ...
    ax2.bar(x, bar_data, color=bar_colors, edgecolor=TEXT_DIM, linewidth=0.5, alpha=0.3)
    ax2.scatter(x, bar_data, c=bar_colors, s=70, zorder=3, edgecolors="white", linewidth=0.8)
    ax2.set_xticks(x)
    ...
    for i, r in enumerate(bio_reports):
        ...
        if r.robustness_random:
            # Individual random trials
            for trial_curve in r.robustness_random_trials:
                ax3.plot(
                    removal_fracs[:len(trial_curve)],
                    trial_curve,
